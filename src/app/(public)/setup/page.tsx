...
import { Trophy, Lock, ShieldAlert, CheckCircle2 } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setError(res.message || 'Error al configurar el administrador.');
      }
    } catch (err) {
      ...
      <div className="w-full max-w-md glass glow-primary rounded-2xl p-8 space-y-8 animate-fade-in">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-zinc-400">
            Crea la cuenta del administrador para inicializar la quiniela.
          </p>
        </div>
        ...
        {success && (
          <div className="p-4 rounded-xl bg-[#10b981]/10 border border-[#10b981]/20 text-[#10b981]/90 text-sm flex items-start space-x-3">
            ...
            <span>¡Administrador creado con éxito! Redirigiendo...</span>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                value="admin"
                disabled
                className="w-full bg-[#13131a] border border-[#1e1e24]/80 rounded-xl py-3 pl-8 pr-4 text-sm font-semibold text-zinc-500 cursor-not-allowed select-none"
              />
            </div>
          </div>
          ...
          >
            {loading ? 'Inicializando...' : 'Confirmar y Empezar'}
          </button>
        </form>
      </div>
    </div>
